...
public class ObjectMapperUtils {
	...
	public static <S, T> Page<T> mapPage(Page<S> source, Class<T> targetClass) {
		...
		for (int i = 0; i < sourceList.size(); i++) {
			T target = ObjectMapperUtils.map(sourceList.get(i), targetClass);
			list.add(target);
		}
		Pageable firstPageWithTwoElements = PageRequest.of(source.getNumber(), source.getSize(), source.getSort());
		Page<T> page = new PageImpl<>(list, firstPageWithTwoElements, source.getTotalElements());
		return page;
